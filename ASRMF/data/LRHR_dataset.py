...
class LRHRDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.netC1.load_state_dict(
            torch.load("/data/wk/Projects/SRMF_3_3/trained_model/decoder_epoch_30.pth",
        ...
        if opt['subset_file'] is not None and opt['phase'] == 'train':
            ...
            self.HR_env, self.paths_HR = util.get_image_paths(opt['data_type'],
                                                              opt['dataroot_HR'])
    ...
    def __getitem__(self, index):
        ...
        if self.opt['phase'] == 'train':
            ...
            H, W, C = img_LR.shape
            LR_size = HR_size // scale  # 24

            # randomly crop
            rnd_h = random.randint(0, max(0, H - LR_size - 1))
            rnd_w = random.randint(0, max(0, W - LR_size - 1))
            img_LR = img_LR[rnd_h:rnd_h + LR_size, rnd_w:rnd_w + LR_size, :]  # 24*24
