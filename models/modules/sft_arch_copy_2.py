...
#         # cond1 = nn.functional.interpolate(cond1, scale_factor=4, mode='bilinear', align_corners=False)
#         # cond2 = self.fea2_CondNet(x[1][2])
#         # cond2 = nn.functional.interpolate(cond2, scale_factor=4, mode='bilinear', align_corners=False)
#         # cond3 = self.fea3_CondNet(x[1][3])
#         # cond3 = nn.functional.interpolate(cond3, scale_factor=4, mode='bilinear', align_corners=False)
#         # cond4 = self.fea4_CondNet(x[1][4])
#         # cond4 = nn.functional.interpolate(cond4, scale_factor=4, mode='bilinear', align_corners=False)
#         cond_seg = self.seg_CondNet(x[1])
#         cond_seg = nn.functional.interpolate(cond_seg, scale_factor=4, mode='bilinear', align_corners=False)
#         fea = self.conv0(x[0])
#         last_fea = fea
#
#         for m in self.sft_branch[0:4]:
#             (fea, cond_seg, cond_seg) = m((fea, cond_seg, cond_seg))
#         for m in self.sft_branch[4:8]:
#             (fea, cond_seg, cond_seg) = m((fea, cond_seg, cond_seg))
#         for m in self.sft_branch[8:12]:
#             (fea, cond_seg, cond_seg) = m((fea, cond_seg, cond_seg))
...
#             (fea, cond_seg, cond_seg) = m((fea, cond_seg, cond_seg))
#
#         fea = self.sft_branch[16]((fea, cond_seg))
#         fea = self.sft_branch[17](fea)
#         # res = self.sft_branch((fea, cond))
#         fea = fea + last_fea
#         out = self.HR_branch(fea)
